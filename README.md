...
| **Verification Time** | <1s for small models | **6-9s for large models** | Realistic timing for full LLM verification |
...
### ⚠️ CORRECTED: Validated Full Pipeline Results

| Model Family | Model A | Model B | Parameters | FAR | FRR | Undecided | Avg Queries | Avg Conf | Time/Query |
|--------------|---------|---------|------------|-----|-----|-----------|-------------|----------|------------|
| **Large Models (7B)** | Llama-2-7B-Chat | Mistral-7B-Instruct | 7B/7B | 0.000 | 0.000 | 0.0% | 32 | 0.990 | 9.06s |
| **Size Fraud** | Pythia-70M | Pythia-160M | 70M/160M | 0.000 | 0.000 | 0.0% | 10 | High | ~6-8s |
| **Identity Control** | Pythia-70M | Pythia-70M | 70M/70M | 0.000 | 0.000 | 0.0% | 10 | High | ~6-8s |

**⚠️ VALIDITY NOTE**: Only the above 3 results are from the complete PoT framework with actual model inference.
**Previous table entries were INVALID** - generated by simplified scripts using direct logit comparison (not full LLM verification).
*Realistic timing: 6-9 seconds per query for proper verification with statistical rigor and ZK proof generation*
...
- **Verification Speed**: 6-9s per query for large models (realistic timing)
