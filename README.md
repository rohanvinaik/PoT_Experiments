...
We generate challenge seeds via HMAC-SHA256(key, run_id || i) → seed_i. This creates a large, deterministic challenge space and prevents cherry-picking.
	2.	Stream responses and update a statistic
For each challenge, we collect both models’ responses and update a running effect-size estimate with an EB confidence interval.
	3.	Stop early when the math is decisive
After each observation, we check explicit decision rules:
...
Otherwise we keep sampling—bounded by mode-specific n_min/n_max.
	4.	Emit an audit bundle
We write seeds, challenge IDs, normalized outputs, decisions, thresholds, and environment metadata. Optionally, we produce a small ZK proof that the verifier computed the published decision from the published transcript.
...
Example runs (illustrative)

Times/queries below are representative examples from real runs in this repo. Use the scripts in Reproduce to generate your own evidence on your hardware.
...
bash scripts/make_evidence_bundle.sh \
  ...
  --include logs/*.json transcripts/*.ndjson configs/*.yaml \
  --include env/pip_freeze.txt env/sysinfo.txt \
  --sign path/to/ed25519_private.pem
