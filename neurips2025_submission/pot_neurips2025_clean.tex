...
\begin{quote}
\colorbox{lightgray}{\parbox{0.95\textwidth}{\textbf{Deployment Reality Check:} Runs on consumer hardware (M1 Max laptop) • Handles production models (34B parameters/206GB) • CI/CD integration ready • No GPU cluster required}}
\end{quote}
...
\begin{quote}
\colorbox{lightgray}{\parbox{0.95\textwidth}{\textbf{Key Insights from Experiments:}
\vspace{-2pt}
\begin{itemize}\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
\item Perfect model discrimination (8/8) with just 14--40 queries
\item Asymmetric verification detects model downgrades/substitutions
\item 30--60× speedup over standard practice (Fixed-N=1000,~\cite{hendrycks2021many})
\item Consumer hardware suffices (no GPU clusters needed)
\end{itemize}}}
\end{quote}
...
\textbf{Note on scale:} The per-challenge scores $X_i$ are clipped to $[0,1]$ for numerical stability. However, the effect sizes $|\overline{X}_n|$ reported in Table~\ref{tab:decisions} are \emph{aggregated metrics} computed as the absolute mean of \emph{unbounded} delta cross-entropies across $n$ challenges, which can exceed 1 when models differ substantially. This allows finer-grained behavioral fingerprinting.
...
\begin{quote}
\colorbox{lightgray}{\parbox{0.95\textwidth}{\textbf{Headline Result:} $30{\times}$--$60{\times}$ faster than fixed-$N$ baselines at matched error; 14--40 queries to decision at $\alpha \in \{0.01, 0.025\}$.}}
\end{quote}
...
\textbf{Key Result}: At~$\alpha \in \{0.01, 0.025\}$, PoT reaches a SAME/DIFF decision in \textbf{17--92\,s} on small models (GPT-2 class), vs \textbf{45--60\,min} for fixed-$N$ baselines (1000 queries), a~\textbf{${\sim}30{\times}$--$200{\times}$} reduction in decision latency. For 7B models, decisions take~\textbf{22--23\,min} with sharding on consumer hardware.

\subsection{Query Efficiency and Error Rates}

\textbf{Statistical Robustness:} Perfect separation (8/8 correct) achieved with minimal data demonstrates exceptional discriminative power. The conservative Wilson CI [0.00, 0.37] reflects sample size, not method uncertainty---the fact that perfect accuracy emerges despite limited testing indicates the behavioral differences are so pronounced that even sparse sampling suffices. This robustness with minimal data is a \emph{strength}: practitioners can verify models with high confidence using minimal queries. A fragile method would show errors even with small samples; our 0/8 error rate demonstrates that behavioral differences between models are sufficiently large for reliable discrimination (see Figures~\ref{fig:time-to-decision} and~\ref{fig:error-rates}).
...
Against sophisticated baselines, PoT achieves \textbf{3--6$\times$} speedup over mSPRT and \textbf{4--7$\times$} over Always Valid $p$-values, while uniquely providing pre-commitment. Table~\ref{tab:decisions} demonstrates: (1) \emph{self-consistency} verification across model sizes (14-30 queries), (2) detection of \emph{architectural differences} (GPT-2 vs DistilGPT-2, GPT-Neo vs Pythia in 32 queries), (3) \emph{scale variants} (GPT-2 vs GPT-2-medium in 40 queries), and (4) \emph{fine-tuning detection} (DialoGPT's conversational adaptation detected in just 16 queries).

\begin{table}[ht!]
\centering
\caption{Model Verification with Actual Experimental Results}
\label{tab:decisions}
\small
\begin{tabular}{@{}l c r r l r@{}}
\toprule
\textbf{Models} & \textbf{Mode} & \textbf{$n$} & \textbf{Time (s)} & \textbf{Classification} & \textbf{Memory (GB)} \\
\midrule
...
llama-7b-base $\to$ llama-7b-base & QUICK & 14 & 1346.7$^{\dagger}$ & SAME & 8.01 \\
llama-7b-chat $\to$ llama-7b-chat & QUICK & 14 & 1381.4$^{\dagger}$ & SAME & 7.95 \\
\midrule
\multicolumn{6}{l}{\emph{Architecture \& scale differences}} \\
gpt2 $\to$ distilgpt2 & AUDIT & 32 & 92.2 & DIFFERENT (distill) & 1.33 \\
gpt2 $\to$ gpt2-medium & AUDIT & 40 & 84.6 & DIFFERENT (scale) & 1.71 \\
gpt-neo $\to$ pythia & AUDIT & 32 & 133.3 & DIFFERENT (arch) & 2.36 \\
\midrule
\multicolumn{6}{l}{\emph{Fine-tuning detection}} \\
dialogpt $\to$ gpt2 & QUICK & 16 & 17.3 & DIFFERENT (tuned) & 1.85 \\
\bottomrule
\end{tabular}

\vspace{3pt}
\footnotesize{$^{\dagger}$7B models on M1 Max with sharding: per-query overhead includes shard cycling}
\end{table}

\subsubsection{Asymmetric Verification Reveals Model Capacity Differences}

A key finding: verification asymmetry provides valuable diagnostic information. Testing GPT-2 as reference against larger GPT-2-medium requires fewer queries (40) than testing in reverse, revealing that the verifier is sensitive to model capacity differences. This asymmetry is a \emph{feature, not a bug}---it enables detection of unauthorized model substitutions or downgrades.

\textbf{Practical guidance:} Always use the \emph{audited} model as reference. The asymmetry inherently captures model capacity differences beyond just training differences, making the verifier sensitive to both architecture changes and parameter reductions.
...
\midrule
\textbf{PoT (ours)} & \textbf{0.3--1.5\,min} & \textbf{0.3--1.5\,min} & \textbf{---} & \textbf{\checkmark} \\
Fixed-$N$ (1000 prompts)$^{[1]}$ & 45--60\,min & 45--60\,min & $30{\times}$--$200{\times}$ & \checkmark \\
Gradient verification$^{[2]}$ & 120\,min & N/A & $80{\times}$--$400{\times}$ & $\times$ \\
\bottomrule
\end{tabular}
...
\textbf{Scope:} This paper demonstrates proof-of-concept on 8 model pairs, sufficient for a workshop paper establishing feasibility. Production deployment would benefit from expanded validation, but the perfect separation achieved suggests the core approach is sound.
...
\section{Broader Impacts \& Conclusion}
...
\textbf{Benefits:} Enables auditing without weight access, supports regulatory compliance, and democratizes verification. \textbf{Risks:} Could aid reverse-engineering; statistical guarantees assume honest reporting. \textbf{Key distinction:} PoT verifies \emph{what} model is served, not \emph{who} serves it---provider authentication requires additional infrastructure like TEEs.
