...
A key finding: verification asymmetry provides valuable diagnostic information. When comparing gpt2$\to$gpt2-medium (RELATED\_TRAINING, $|\overline{X}_n|=3.553$) versus gpt2-medium$\to$gpt2 (DIFFERENT\_TRAINING, $|\overline{X}_n|=8.450$), the higher divergence in the reverse direction reveals that smaller models cannot replicate larger models' nuanced representations. This asymmetry is a \emph{feature, not a bug}---it enables detection of unauthorized model downgrades or capacity-reducing substitutions.
