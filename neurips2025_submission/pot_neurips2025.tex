...
\textbf{dozens of queries} (perfect separation in experiments with just 14--48 queries) 
...
\item Perfect model discrimination (8/8) with just 14--48 queries
\item Asymmetric verification detects model downgrades/substitutions
\item 30× speedup over standard practice (Fixed-N=1000,~\cite{hendrycks2021many})
...
gpt2 $\to$ distilgpt2 & AUDIT & 12.968 & 32 & DIFFERENT (arch) & 61.4 \\
gpt2 $\to$ gpt2-medium & AUDIT & 1.728 & 64 & DIFFERENT (scale) & 84.6 \\
gpt-neo-125m $\to$ pythia-160m & AUDIT & 6.172 & 32 & DIFFERENT (arch) & 91.2 \\
\midrule
\multicolumn{6}{l}{\emph{Fine-tuning detection}} \\
dialogpt $\to$ gpt2 & QUICK & 20.680 & 16 & DIFFERENT (tuned) & 42.1 \\
\bottomrule
\end{tabular}

\vspace{3pt}
\footnotesize{$^{\dagger}$M1 Max with sharding: model loads/unloads per query; average time includes sharding overhead}
\end{table}

\subsubsection{Asymmetric Verification Reveals Model Capacity Differences}

A key finding: verification asymmetry provides valuable diagnostic information. GPT-2$\to$GPT-2-medium shows $|\overline{X}_n|=1.728$ while the reverse shows $|\overline{X}_n|=6.675$ (from our experiments), revealing that smaller models cannot replicate larger models' nuanced representations. This asymmetry is a \emph{feature, not a bug}---it enables detection of unauthorized model substitutions or downgrades.
