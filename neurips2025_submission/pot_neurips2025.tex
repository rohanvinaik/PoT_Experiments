...
From recent experimental runs, verification reaches decisions in \textbf{14--48} queries with zero observed errors on~$n=8$ tested pairs (0/8 errors, Wilson 95\% CI: [0.00, 0.37]$^*$, see Figure~\ref{fig:time-to-decision} for time-to-decision trajectories). $^*$\footnotesize{Small-$n$ zero errors reported with Wilson confidence interval for completeness.} Against a \textbf{fixed-$N$=1000} baseline (standard for behavioral test sets), this represents \textbf{95.2--98.6\%} query reduction. QUICK mode ($\alpha=0.025$, $n_{\max}=120$) averages 15 queries; AUDIT mode ($\alpha=0.01$, $n_{\max}=400$) averages 32 queries.
