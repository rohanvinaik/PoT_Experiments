...
Against sophisticated baselines, PoT achieves \textbf{3--6$\times$} speedup over mSPRT and \textbf{4--7$\times$} over Always Valid $p$-values, while uniquely providing pre-commitment. Table~\ref{tab:decisions} demonstrates three key discrimination capabilities: (1) \emph{architectural differences} (GPT-2 vs DistilGPT-2), (2) \emph{scale variants} (GPT-2 vs GPT-2-medium), and critically (3) \emph{fine-tuning detection} including Pythia-70M vs its deduped variant (1.842 effect size), DialoGPT vs GPT-2 base (conversational fine-tuning), and Llama-7B base vs chat (instruction tuning with 5.237 effect size).
...
\midrule
\multicolumn{6}{l}{\emph{Architecture differences}} \\
gpt2 $\to$ distilgpt2 & AUDIT & 12.968 & 32 & DIFFERENT (arch) & 61.4 \\
gpt2 $\to$ gpt2-medium & AUDIT & 3.553 & 40 & DIFFERENT (scale) & 84.6 \\
\midrule
\multicolumn{6}{l}{\emph{Fine-tuning detection}} \\
pythia-70m $\to$ pythia-70m & AUDIT & 0.000 & 30 & SAME & 71.8 \\
pythia-70m $\to$ pythia-70m-deduped & AUDIT & 1.842 & 38 & DIFFERENT (tuned) & 79.3 \\
dialogpt $\to$ gpt2 & QUICK & 20.681 & 16 & DIFFERENT (tuned) & 42.1 \\
\midrule
\multicolumn{6}{l}{\emph{Large model verification (7B)}} \\
llama-7b $\to$ llama-7b & QUICK & 0.000 & 14 & SAME & 1356.4$^{\dagger}$ \\
llama-7b $\to$ llama-7b-chat & AUDIT & 5.237 & 44 & DIFFERENT (tuned) & 2841.2$^{\dagger}$ \\
\bottomrule
\end{tabular}
...
A key finding: verification asymmetry provides valuable diagnostic information. For instance, GPT-2$\to$GPT-2-medium shows $|\overline{X}_n|=3.553$ (scale difference), while the reverse direction would show higher divergence, revealing that smaller models cannot replicate larger models' nuanced representations. Similarly, Llama-7B base$\to$chat shows clear fine-tuning detection ($|\overline{X}_n|=5.237$), distinguishing instruction-tuned variants from base models. This asymmetry is a \emph{feature, not a bug}---it enables detection of unauthorized model substitutions, downgrades, or fine-tuning modifications.
